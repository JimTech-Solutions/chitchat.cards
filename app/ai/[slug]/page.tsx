...
const Page: React.FC = () => {
    ...
    useEffect(() => {
        ...
        let { data: generated_questions, error } = await supabase
        .from('generated_questions')
        .select('*').eq('game_id', slug).single()

        // console.log(result);

        if (generated_questions) {
            console.log(generated_questions.questions);
            setQuestions(shuffleArray(generated_questions.questions));
        } else {
            console.log(error);
        }
                    ...
                    <Slider ref={sliderRef}
                        ...
                        {questions.map((question, index) => {
                            ...
                            <div key={index} className="text-center p-10 my-auto justify-center items-center">
                                    <p className={`mb-3`} style={{ color: textColor }}>{question.category.toUpperCase()}</p>
